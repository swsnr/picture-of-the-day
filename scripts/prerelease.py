...
def assert_no_releasenotes(new_version: Version):
    metadata_file = Path(os.environ['CRATE_ROOT']) / 'de.swsnr.pictureoftheday.metainfo.xml'
...
def update_releasenotes(new_version: Version, *, tag_name: str, date: str, dry_run: bool):
    ...
    if url is None:
        # Add URL element, preceeded by necessary indentation.
        next_release[-1].tail = next_release.text
        url = etree.SubElement(next_release, 'url')
        url.tail = next_release.tail
    url.text = f'https://codeberg.org/swsnr/picture-of-the-day/releases/tag/{tag_name}'
    if dry_run:
        etree.dump(tree)
    else:
